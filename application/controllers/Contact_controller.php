...
class Contact_controller extends CI_Controller
{
    ...
    public function index()
    {
        ...
        $search        = $this->contact_model->searchterm_handler($this->input->post('search', true));

        $total_records = $this->contact_model->get_total($search);

        $page = ($this->uri->segment(2)) ? ($this->uri->segment(2) - 1) : 0;

        if ($total_records > 0) {

            $data['data'] = $this->contact_model->getContacts($limit_per_page, $page * $limit_per_page, $search);
    ...
    public function delete($id)
    {
        $id   = $this->uri->segment(2);
        if(! $this->hasPermission($id)) redirect('contact');
        $item = $this->contact_model->delete($id);
